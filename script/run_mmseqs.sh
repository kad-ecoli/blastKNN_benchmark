...
for dataset in `echo validate`;do
    mkdir -p $dbdir/result/tmp
    $bindir/mmseqs/bin/mmseqs easy-search $dbdir/curated/sequence/$dataset.fasta $dbdir/curated/sequence/uniprot_sprot_exp mmseqs_$dataset.m6 $dbdir/result/tmp --format-output query,qlen,target,tlen,evalue,bits,alnlen,fident
    rm -rf $dbdir/result/tmp
