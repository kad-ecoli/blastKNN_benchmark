...
method_list=["10","25","50","100","200","500","1000"]
...
for metric in ["Fmax","wFmax"]:
    ...
    for m,method in enumerate(method_list):
        ...
        for s,(label,color) in enumerate(scoring_list):
            filename="%s/result/blastp_%s_%s_validate.txt"%(rootdir,label,method)
    ...
    plt.savefig("%s/result/tune_blastp_%s.png"%(rootdir,metric),dpi=300)
    ...
    for m,method in enumerate(method_list):
        ...
        for s in range(len(data[m])):
            txt+="%.3f\t"%data[m][s]
        txt=txt[:-1]+'\n'
fp=open("%s/result/tune_blastp.tsv"%(rootdir),'w')
fp.write(txt)
fp.close()
...
plt.tight_layout(pad=0.9,w_pad=0.1,h_pad=0.1)
plt.savefig("%s/result/tune_blastp.png"%(rootdir),dpi=300)
